...
@PersistenceContext private EntityManager em;
    ...
    public List getMachinesNaam() {
        List ma = em.createQuery("SELECT m.mnaam, m.mid FROM Machines m").getResultList();
        return ma;
    }
    @Override
    ...
    public List getResLoginMid(int m) {
        List res = em.createQuery("SELECT r.rid, r.login.login FROM Reservaties r WHERE r.moid IN (SELECT m FROM Momenten m WHERE m.mid = :mid)").setParameter("mid", getMachineMid(m)).getResultList();
        return res;
    }
    @Override
